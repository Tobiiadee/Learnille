/** @format */

import Card from "@/components/ui/Card/Card";
import { useState } from "react";
import { z } from "zod";
import { SubmitHandler, useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";

const passwordSchema = z
  .object({
    currentPassword: z.string().min(8),
  })
  .extend({
    newPassword: z
      .string()
      .min(8, { message: "Password must be at least 8 characters" }),
    confirmPassword: z
      .string()
      .min(8, { message: "Password must be at least 8 characters" }),
  })
  .refine((data) => data.newPassword === data.confirmPassword, {
    message: "Passwords don't match",
    path: ["newPassword"],
  });

type PasswordType = z.infer<typeof passwordSchema>;

export default function Change_Password() {
  const [viewCurrentPassword, setViewCurrentPassword] = useState(false);
  const [viewNewPassword, setViewNewPassword] = useState(false);
  const [viewConfirmPassword, setViewConfirmPassword] = useState(false);

  const [oldPasswordMatched, setOldPasswordMatched] = useState(false);

  const viewCurrentPasswordHandler = () =>
    setViewCurrentPassword((prev) => !prev);
  const viewNewPasswordHandler = () => setViewNewPassword((prev) => !prev);
  const viewConfirmPasswordHandler = () =>
    setViewConfirmPassword((prev) => !prev);

  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<PasswordType>({
    resolver: zodResolver(passwordSchema),
  });

  const savePassword: SubmitHandler<PasswordType> = (passwordData) => {
    const passwords = {
      currentPassword: passwordData.currentPassword,
      newPassword: passwordData.newPassword,
      confirmPassword: passwordData.confirmPassword,
    };

    if (passwordData.currentPassword === passwordData.newPassword) {
      setOldPasswordMatched(true);
      return;
    } else {
      setOldPasswordMatched(false);
    }

    console.log(passwords);
  };

  return (
    <Card>
      <form
        onSubmit={handleSubmit(savePassword)}
        className='flex flex-col gap-4'>
        <h2 className='text-sm font-semibold'>Change Password</h2>
        <section className='flex flex-col gap-4'>
          <section>
            <div className='flex flex-col gap-1 w-11/12'>
              <label htmlFor='current_password' className='font-normal text-sm'>
                Current Password
              </label>
              <div className='border rounded-md h-9 px-2 py-1 flex gap-2 items-center'>
                <input
                  type={viewCurrentPassword ? "text" : "password"}
                  {...register("currentPassword")}
                  id='current_password'
                  placeholder='Password'
                  className='text-black text-sm w-full outline-none placeholder:font-normal'
                />
                <button
                  type='button'
                  onClick={viewCurrentPasswordHandler}
                  className=''>
                  {""}
                  {viewCurrentPassword ? (
                    <svg
                      width='20px'
                      height='20px'
                      viewBox='0 0 24 24'
                      fill='none'
                      xmlns='http://www.w3.org/2000/svg'>
                      <path
                        d='M1 12C1 12 5 4 12 4C19 4 23 12 23 12'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                      <path
                        d='M1 12C1 12 5 20 12 20C19 20 23 12 23 12'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                      <circle
                        cx='12'
                        cy='12'
                        r='3'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                    </svg>
                  ) : (
                    <svg
                      width='20px'
                      height='20px'
                      viewBox='0 0 28 28'
                      fill='none'
                      xmlns='http://www.w3.org/2000/svg'>
                      <path
                        clipRule='evenodd'
                        d='M22.6928 1.55018C22.3102 1.32626 21.8209 1.45915 21.6 1.84698L19.1533 6.14375C17.4864 5.36351 15.7609 4.96457 14.0142 4.96457C9.32104 4.96457 4.781 7.84644 1.11993 13.2641L1.10541 13.2854L1.09271 13.3038C0.970762 13.4784 0.967649 13.6837 1.0921 13.8563C3.79364 17.8691 6.97705 20.4972 10.3484 21.6018L8.39935 25.0222C8.1784 25.4101 8.30951 25.906 8.69214 26.1299L9.03857 26.3326C9.4212 26.5565 9.91046 26.4237 10.1314 26.0358L23.332 2.86058C23.553 2.47275 23.4219 1.97684 23.0392 1.75291L22.6928 1.55018ZM18.092 8.00705C16.7353 7.40974 15.3654 7.1186 14.0142 7.1186C10.6042 7.1186 7.07416 8.97311 3.93908 12.9239C3.63812 13.3032 3.63812 13.8561 3.93908 14.2354C6.28912 17.197 8.86102 18.9811 11.438 19.689L12.7855 17.3232C11.2462 16.8322 9.97333 15.4627 9.97333 13.5818C9.97333 11.2026 11.7969 9.27368 14.046 9.27368C15.0842 9.27368 16.0317 9.68468 16.7511 10.3612L18.092 8.00705ZM15.639 12.3137C15.2926 11.7767 14.7231 11.4277 14.046 11.4277C12.9205 11.4277 12 12.3906 12 13.5802C12 14.3664 12.8432 15.2851 13.9024 15.3624L15.639 12.3137Z'
                        fill='#000000'
                        fillRule='evenodd'
                      />
                      <path
                        d='M14.6873 22.1761C19.1311 21.9148 23.4056 19.0687 26.8864 13.931C26.9593 13.8234 27 13.7121 27 13.5797C27 13.4535 26.965 13.3481 26.8956 13.2455C25.5579 11.2677 24.1025 9.62885 22.5652 8.34557L21.506 10.2052C22.3887 10.9653 23.2531 11.87 24.0894 12.9239C24.3904 13.3032 24.3904 13.8561 24.0894 14.2354C21.5676 17.4135 18.7903 19.2357 16.0254 19.827L14.6873 22.1761Z'
                        fill='#000000'
                      />
                    </svg>
                  )}
                </button>
              </div>
            </div>
          </section>

          <section>
            <div className='flex flex-col gap-1 w-11/12'>
              <label htmlFor='new_password' className='font-normal text-sm'>
                New Password
              </label>
              <div
                className={`${errors.newPassword ? "border-[#e74c3c]" : ""} border rounded-md h-9 px-2 py-1 flex gap-2 items-center`}>
                <input
                  type={viewNewPassword ? "text" : "password"}
                  {...register("newPassword")}
                  id='new_password'
                  placeholder='Password'
                  className='text-black text-sm w-full outline-none placeholder:font-normal'
                />
                <button
                  type='button'
                  onClick={viewNewPasswordHandler}
                  className=''>
                  {""}
                  {viewNewPassword ? (
                    <svg
                      width='20px'
                      height='20px'
                      viewBox='0 0 24 24'
                      fill='none'
                      xmlns='http://www.w3.org/2000/svg'>
                      <path
                        d='M1 12C1 12 5 4 12 4C19 4 23 12 23 12'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                      <path
                        d='M1 12C1 12 5 20 12 20C19 20 23 12 23 12'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                      <circle
                        cx='12'
                        cy='12'
                        r='3'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                    </svg>
                  ) : (
                    <svg
                      width='20px'
                      height='20px'
                      viewBox='0 0 28 28'
                      fill='none'
                      xmlns='http://www.w3.org/2000/svg'>
                      <path
                        clipRule='evenodd'
                        d='M22.6928 1.55018C22.3102 1.32626 21.8209 1.45915 21.6 1.84698L19.1533 6.14375C17.4864 5.36351 15.7609 4.96457 14.0142 4.96457C9.32104 4.96457 4.781 7.84644 1.11993 13.2641L1.10541 13.2854L1.09271 13.3038C0.970762 13.4784 0.967649 13.6837 1.0921 13.8563C3.79364 17.8691 6.97705 20.4972 10.3484 21.6018L8.39935 25.0222C8.1784 25.4101 8.30951 25.906 8.69214 26.1299L9.03857 26.3326C9.4212 26.5565 9.91046 26.4237 10.1314 26.0358L23.332 2.86058C23.553 2.47275 23.4219 1.97684 23.0392 1.75291L22.6928 1.55018ZM18.092 8.00705C16.7353 7.40974 15.3654 7.1186 14.0142 7.1186C10.6042 7.1186 7.07416 8.97311 3.93908 12.9239C3.63812 13.3032 3.63812 13.8561 3.93908 14.2354C6.28912 17.197 8.86102 18.9811 11.438 19.689L12.7855 17.3232C11.2462 16.8322 9.97333 15.4627 9.97333 13.5818C9.97333 11.2026 11.7969 9.27368 14.046 9.27368C15.0842 9.27368 16.0317 9.68468 16.7511 10.3612L18.092 8.00705ZM15.639 12.3137C15.2926 11.7767 14.7231 11.4277 14.046 11.4277C12.9205 11.4277 12 12.3906 12 13.5802C12 14.3664 12.8432 15.2851 13.9024 15.3624L15.639 12.3137Z'
                        fill='#000000'
                        fillRule='evenodd'
                      />
                      <path
                        d='M14.6873 22.1761C19.1311 21.9148 23.4056 19.0687 26.8864 13.931C26.9593 13.8234 27 13.7121 27 13.5797C27 13.4535 26.965 13.3481 26.8956 13.2455C25.5579 11.2677 24.1025 9.62885 22.5652 8.34557L21.506 10.2052C22.3887 10.9653 23.2531 11.87 24.0894 12.9239C24.3904 13.3032 24.3904 13.8561 24.0894 14.2354C21.5676 17.4135 18.7903 19.2357 16.0254 19.827L14.6873 22.1761Z'
                        fill='#000000'
                      />
                    </svg>
                  )}
                </button>
              </div>
              {oldPasswordMatched && (
                <h2 className='text-xs text-[#e74c3c]'>
                  Set a different password. New password similar to your old
                  password!
                </h2>
              )}
            </div>
          </section>

          <section>
            <div className='flex flex-col gap-1 w-11/12'>
              <label htmlFor='confirm_password' className='font-normal text-sm'>
                Confirm Password
              </label>
              <div
                className={`${errors.newPassword ? "border-[#e74c3c]" : ""} border rounded-md h-9 px-2 py-1 flex gap-2 items-center`}>
                <input
                  type={viewConfirmPassword ? "text" : "password"}
                  {...register("confirmPassword")}
                  id='confirm_password'
                  placeholder='Password'
                  className='text-black text-sm w-full outline-none placeholder:font-normal'
                />
                <button
                  type='button'
                  onClick={viewConfirmPasswordHandler}
                  className=''>
                  {""}
                  {viewConfirmPassword ? (
                    <svg
                      width='20px'
                      height='20px'
                      viewBox='0 0 24 24'
                      fill='none'
                      xmlns='http://www.w3.org/2000/svg'>
                      <path
                        d='M1 12C1 12 5 4 12 4C19 4 23 12 23 12'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                      <path
                        d='M1 12C1 12 5 20 12 20C19 20 23 12 23 12'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                      <circle
                        cx='12'
                        cy='12'
                        r='3'
                        stroke='#000000'
                        strokeWidth='1.7'
                        strokeLinecap='round'
                        strokeLinejoin='round'
                      />
                    </svg>
                  ) : (
                    <svg
                      width='20px'
                      height='20px'
                      viewBox='0 0 28 28'
                      fill='none'
                      xmlns='http://www.w3.org/2000/svg'>
                      <path
                        clipRule='evenodd'
                        d='M22.6928 1.55018C22.3102 1.32626 21.8209 1.45915 21.6 1.84698L19.1533 6.14375C17.4864 5.36351 15.7609 4.96457 14.0142 4.96457C9.32104 4.96457 4.781 7.84644 1.11993 13.2641L1.10541 13.2854L1.09271 13.3038C0.970762 13.4784 0.967649 13.6837 1.0921 13.8563C3.79364 17.8691 6.97705 20.4972 10.3484 21.6018L8.39935 25.0222C8.1784 25.4101 8.30951 25.906 8.69214 26.1299L9.03857 26.3326C9.4212 26.5565 9.91046 26.4237 10.1314 26.0358L23.332 2.86058C23.553 2.47275 23.4219 1.97684 23.0392 1.75291L22.6928 1.55018ZM18.092 8.00705C16.7353 7.40974 15.3654 7.1186 14.0142 7.1186C10.6042 7.1186 7.07416 8.97311 3.93908 12.9239C3.63812 13.3032 3.63812 13.8561 3.93908 14.2354C6.28912 17.197 8.86102 18.9811 11.438 19.689L12.7855 17.3232C11.2462 16.8322 9.97333 15.4627 9.97333 13.5818C9.97333 11.2026 11.7969 9.27368 14.046 9.27368C15.0842 9.27368 16.0317 9.68468 16.7511 10.3612L18.092 8.00705ZM15.639 12.3137C15.2926 11.7767 14.7231 11.4277 14.046 11.4277C12.9205 11.4277 12 12.3906 12 13.5802C12 14.3664 12.8432 15.2851 13.9024 15.3624L15.639 12.3137Z'
                        fill='#000000'
                        fillRule='evenodd'
                      />
                      <path
                        d='M14.6873 22.1761C19.1311 21.9148 23.4056 19.0687 26.8864 13.931C26.9593 13.8234 27 13.7121 27 13.5797C27 13.4535 26.965 13.3481 26.8956 13.2455C25.5579 11.2677 24.1025 9.62885 22.5652 8.34557L21.506 10.2052C22.3887 10.9653 23.2531 11.87 24.0894 12.9239C24.3904 13.3032 24.3904 13.8561 24.0894 14.2354C21.5676 17.4135 18.7903 19.2357 16.0254 19.827L14.6873 22.1761Z'
                        fill='#000000'
                      />
                    </svg>
                  )}
                </button>
              </div>
              {errors.newPassword && (
                <h2 className='text-xs text-[#e74c3c]'>
                  {errors.newPassword.message}
                </h2>
              )}
            </div>
          </section>
        </section>

        <button
          type='submit'
          className={`self-start flex justify-between bg-[#edf1ff]  active:bg-[#ced5ee] transition duration-300`}>
          <span className='text-[8px] self-center tab:text-xs text-layout-bg font-semibold py-1 px-2 '>
            Save Password
          </span>
          <span className='text-white py-2 px-2  bg-layout-bg rounded-r-md '>
            <svg
              xmlns='http://www.w3.org/2000/svg'
              fill='none'
              viewBox='0 0 24 24'
              strokeWidth={1.5}
              stroke='currentColor'
              className='w-4 h-4'>
              <path
                strokeLinecap='round'
                strokeLinejoin='round'
                d='M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3'
              />
            </svg>
          </span>
        </button>
      </form>
    </Card>
  );
}
